...
for i=1:73                  %IO信息，1:73是io口的数量，不同的电路io口数量不一样，从data文件里找有多少io口，从哪里开始这些信息来设置索引。
    iodata{i} = data{i+6};
end
logidata = {};              %逻辑连线
for i=1:668                 %同上
    logidata{i} = data{i+198};  %这是加198是因为ligidata信息是从data中的第199行开始的。
end
...
input = io(1:41);            %输入io编号
output = io(42:length(io));          %输出io编号

lut_info = {};
for i=1:61
    lut_info{i} = data{i+84};
end
