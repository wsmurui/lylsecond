...
for i=1:7                  %IO信息，1:7是io口的数量，不同的电路io口数量不一样，你可以从data文件里找有多少io口，从哪里开始这些信息来设置索引。
    iodata{i} = data{i+6};
end
logidata = {};
for i=1:18                  %同上
    logidata{i} = data{i+73};  %这是加73是因为ligidata信息是从data中的第74行开始的。
end
...
input = io(1:5);            %输入io编号
output = io(6:length(io));          %输出io编号

lut_info = {};
for i=1:2
    lut_info{i} = data{i+18};
end
...
count.lut_input_info = nan(6,length(lut_info));     %初始值为nan的数组（矩阵）
count.output_p = nan(1,length(output));         

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% count.input_p = [0.2 0.5 0.8 0.1 0.6];          %每个输入为1的概率，如果你要改变各个输入口初始为1的概率就在这里改，因为
